...
export class ItemSheetPF_Container extends ItemSheetPF {
  ...
  async getData() {
    ...
    data.weightUnits = game.settings.get("pf1", "units") === "metric" ? game.i18n.localize("PF1.Kgs") : game.i18n.localize("PF1.Lbs")
  ...
  _prepareContents(data) {
    ...
    const inventory = {
      weapon: { label: game.i18n.localize("PF1.InventoryWeapons"), canCreate: true, hasActions: false, items: [], canEquip: false, dataset: { type: "weapon" } },
      equipment: { label: game.i18n.localize("PF1.InventoryArmorEquipment"), canCreate: true, hasActions: true, items: [], canEquip: false, dataset: { type: "equipment" }, hasSlots: true },
  ...
  async _updateItems() {
    ...
    for (const data of updates) {
      const item = this.item.items.filter(o => { return o._id === data._id; })[0];
