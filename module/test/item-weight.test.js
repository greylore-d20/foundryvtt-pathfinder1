          ...
          for (const kind of ["world", "embedded"]) {
            describe(`${kind.capitalize()} item 'Acid'`, function () {
              ...
              before(async () => {
                ...
                await item.update({ "data.quantity": 1, "data.weight.value": 1 });
              ...
              describe("with a quantity of 2", function () {
                before(async () => {
                  await item.update({ "data.quantity": 2 });
                });
                ...
                describe("and a weight value of 10", function () {
                  before(async () => {
                    await item.update({ "data.weight.value": 10 });
                  });
...
const getCarriedPresentationForWeight = (weight) => {
  ...
  const weightLabel = usystem === "metric" ? "PF1.CarryLabelKg" : "PF1.CarryLabel";
  return game.i18n.localize(weightLabel).format(Math.roundDecimals(convertWeight(weight), 1));
};
